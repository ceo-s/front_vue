    ...
    <div class="profile">
        ...
        <div class="box bio">
            ...
            <div class="bio-content">
                <h1>{{ client.name }}</h1>
                <a :href="`https://t.me/${client.telegram}`"
                target="_blank" rel="noopener noreferrer">@{{ client.telegram }}</a>
            </div>
        </div>
...
export default {
    ...
    methods: {
        async fetchClient() {
            await axios.get("http://127.0.0.1:8000/api/clients/" + this.clientId + "/")
                .then((resp) => {
        ...
        async fetchClientStats() {
            await axios.get("http://127.0.0.1:8000/api/clientstats_active/" + this.clientId + "/")
                .then((resp) => {
        ...
        async fetchClientBaseExercises() {
            await axios.get("http://127.0.0.1:8000/api/client_base_exercises/" + this.clientId + "/")
                .then((resp) => {
