<template>
  <div class="selects">
    <pop-up-select @update:option="fetchCycles" :options="clients">
    ...
    <pop-up-select @update:option="fetchPrograms" :options="cycles">
      <template v-slot:title>
        <plus-svg class="plus" />
        <p>Программы:</p>
      </template>
...
export default {
  ...
  methods: {
    ...
    async fetchCycles(client_id) {
      this.currentClientId = client_id;
      await axios
        .get("http://127.0.0.1:8000/api/cycles/filter/", {
