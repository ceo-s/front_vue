<template>
  <transition name="appear">
    <div style="z-index: 1; position: fixed" v-if="$store.state.popUps.auth">
      <pop-up-back @update:visible="close">
        <pop-up class="popup">
          <div class="buttons">
            <my-button :class="{ hide: !login }" @click="login = true"
              >Login</my-button
            >
            <my-button :class="{ hide: login }" @click="login = false"
...
export default {
  data() {
    return {
      login: true,
      popOutSize:
        "width: 75%; height: 350px; padding-bottom: 20px; min-width: 300px;",
    };
  },
  components: {
    LoginForm,
